#!/usr/bin/env python3
"""
SAIL to CGEN - Enhanced Converter with Pretty Printing
Converter from YAML/JSON to S-expressions with formatted output
...
class SExpressionConverter:
    """Converts structured data to S-expression format with pretty printing"""
    
    def __init__(self, namespace_prefix: str = "yaml", pretty_print: bool = False):
    ...
    def _convert_dict(self, data: Dict, key: str = None) -> str:
        ...
        content = ' '.join(items) if not self.pretty_print else self._format_items(items)
    ...
    def _format_items(self, items: List[str]) -> str:
        """Format items with proper indentation for pretty printing"""
        if not self.pretty_print:
            return ' '.join(items)
        
        self.indent_level += 1
        indent = '\n' + ' ' * (self.indent_level * self.indent_size)
        formatted = indent + indent.join(items)
        self.indent_level -= 1
        return formatted + '\n' + ' ' * (self.indent_level * self.indent_size)
...
def main():
    """Main function"""
    if len(sys.argv) < 2:
        print("Usage: python pretty_converter.py <input_file> [--pretty]")
